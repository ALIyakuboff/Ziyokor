...
async function run() {
    try {
        ...
        // Check if vDate is string and needs parsing, usually Postgres returns Date for date/timestamp
        if (typeof vDate === 'string' && vDate.includes('T')) vDate = DateTime.fromISO(vDate).setZone(APP_TZ).toISODate()!;
        ...
        if (row.status === 'missed' && vDate === yesterday) {
            ...
            console.error("❌ FAILURE: Mandatory task status or date is wrong. Status:", row.status, "Date:", vDate);
        }
        ...
        // Query using the REPORT logic
        // We can't easily import the route logic directly, but we can verify the SQL query part
        // which matches what we put in admin.routes.ts
        const reportQuery = `
            SELECT t.title, 
        ...
        if (r2.rows.length === 0) {
            console.error("❌ FAILURE: Report task not found.");
        } else {
