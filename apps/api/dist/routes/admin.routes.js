...
exports.adminRouter.use(auth_1.authRequired);
// Create Recurring Task Template (Continuing Mandatory)
exports.adminRouter.post("/templates", (0, auth_1.requireRole)("admin"), async (req, res, next) => {
    try {
        const body = (0, validate_1.mustParse)(zod_1.z.object({
            titles: zod_1.z.array(zod_1.z.string().min(1)).min(1),
            user_ids: zod_1.z.array(zod_1.z.string().uuid()).min(1),
            is_mandatory: zod_1.z.boolean().default(true),
            recurrence: zod_1.z.enum(["daily"]).default("daily")
        }), req.body);
        const me = req.user;
        let createdCount = 0;
        for (const uid of body.user_ids) {
            for (const title of body.titles) {
                await (0, db_1.query)(`INSERT INTO mandatory_task_templates 
                     (user_id, title, recurrence_type, is_active, created_by_admin_id, is_mandatory)
                     VALUES ($1, $2, $3, true, $4, $5)`, [uid, title, body.recurrence, me.id, body.is_mandatory]);
                createdCount++;
            }
        }
        ...
        const today = (0, date_1.todayISO)();
        await (0, generateMandatory_1.generateMandatoryJob)(today);
...
exports.adminRouter.get("/workers", (0, auth_1.requireRole)("admin"), async (_req, res, next) => {
    try {
        const r = await (0, db_1.query)("SELECT id, full_name FROM users WHERE (role='worker' OR role='admin') AND is_active=true ORDER BY full_name ASC");
...
exports.adminRouter.post("/workers", (0, auth_1.requireRole)("admin"), async (req, res, next) => {
    try {
        const body = (0, validate_1.mustParse)(zod_1.z.object({
            full_name: zod_1.z.string().min(2).max(120),
            phone_login: zod_1.z.string().min(7).max(30)
        }), req.body);
        const phone = (0, phone_1.normalizePhoneDigits)(body.phone_login);
        if (!phone || phone.length < 4)
            return res.status(400).json({ error: "INVALID_PHONE" });
        const exists = await (0, db_1.query)("SELECT id FROM users WHERE phone_login=$1 LIMIT 1", [phone]);
        if (exists.rows.length)
            return res.status(409).json({ error: "PHONE_ALREADY_EXISTS" });
        const pwd = (0, phone_1.last4Digits)(phone);
        const hash = await bcryptjs_1.default.hash(pwd, 10);
        const ins = await (0, db_1.query)(`INSERT INTO users (full_name, phone_login, password_hash, role, is_active)
       VALUES ($1,$2,$3,'worker',true)
       RETURNING id, full_name, phone_login, role, is_active`, [body.full_name, phone, hash]);
...
// Deactivate worker
exports.adminRouter.patch("/workers/:id/deactivate", (0, auth_1.requireRole)("admin"), async (req, res, next) => {
    try {
        const id = req.params.id;
        await (0, db_1.query)("UPDATE users SET is_active=false WHERE id=$1", [id]);
        res.json({ ok: true });
    }
    catch (e) {
        next(e);
...
exports.adminRouter.get("/workers/:id/week", (0, auth_1.requireRole)("admin"), async (req, res, next) => {
    try {
        const workerId = req.params.id;
        const anchor = req.query.anchor || undefined;
        const date = anchor && /^\d{4}-\d{2}-\d{2}$/.test(anchor) ? anchor : undefined;
        const days = (0, date_1.weekDaysMonToSat)(date || (0, date_1.todayISO)());
        if (!days)
            return res.status(400).json({ error: "INVALID_DATE" });
        const u = await (0, db_1.query)("SELECT id, full_name FROM users WHERE id=$1 LIMIT 1", [workerId]);
        if (!u.rows.length)
            return res.status(404).json({ error: "WORKER_NOT_FOUND" });
        await (0, tasks_routes_1.syncCarryovers)(workerId);
       ...
       ORDER BY t.visible_date ASC,
         CASE
           ...
           ELSE 2
         END ASC,
         t.created_at ASC`, [workerId, days]);
        const grouped = {};
        for (const d of days) {
...
exports.adminRouter.post("/mandatory/one-off", async (req, res, next) => {
    try {
        const body = (0, validate_1.mustParse)(zod_1.z.object({
            user_id: zod_1.z.string().min(1),
            title: zod_1.z.string().min(1).max(200),
            date: validate_1.zISODate
        }), req.body);
        const closed = await (0, db_1.query)("SELECT date FROM day_closures WHERE date=$1 LIMIT 1", [body.date]);
       ...
       RETURNING *`, [body.user_id, body.title, body.date]);
        const task = ins.rows[0];
        // Emit real-time event to the worker
        (0, socket_1.emitToUser)(body.user_id, "task:created", { task });
        res.json({ task });
    }
    catch (e) {
        next(e);
...
exports.adminRouter.delete("/tasks/:id", (0, auth_1.requireRole)("admin"), async (req, res, next) => {
    try {
        const id = req.params.id;
        const today = (0, date_1.todayISO)();
        // Get task info before deleting
        const taskResult = await (0, db_1.query)("SELECT * FROM tasks WHERE id=$1 AND deleted_at IS NULL LIMIT 1", [id]);
        ...
        if (taskDate instanceof Date) {
            taskDate = luxon_1.DateTime.fromJSDate(taskDate).setZone(date_1.APP_TZ).toISODate();
        }
        // Restriction: Only delete today or future
        if (taskDate < today) {
            return res.status(403).json({ error: "CANNOT_DELETE_PAST_TASKS" });
        }
        await (0, db_1.query)(`UPDATE tasks SET deleted_at=NOW() WHERE id=$1 AND deleted_at IS NULL`, [id]);
        // Emit real-time event to the task owner
        (0, socket_1.emitToUser)(task.user_id, "task:deleted", { taskId: id });
        res.json({ ok: true });
    }
    catch (e) {
        next(e);
...
exports.adminRouter.get("/analytics", async (req, res, next) => {
    ...
    try {
        ...
        const qv = (0, validate_1.mustParse)(zod_1.z.object({
            range: zod_1.z.enum(["week", "month", "year"]),
            anchor: validate_1.zISODate.optional(),
        ...
        const anchorISO = qv.anchor || (0, date_1.todayISO)();
        const anchorDT = luxon_1.DateTime.fromISO(anchorISO, { zone: date_1.APP_TZ });
        if (!anchorDT.isValid)
            return res.status(400).json({ error: "INVALID_DATE" });
        const range = qv.range;
        // Sync carryovers for the target worker before calculating analytics
        if (targetWorkerId) {
            await (0, tasks_routes_1.syncCarryovers)(targetWorkerId);
        }
        // Build buckets for series
        const buckets = [];
        if (range === "week") {
            // 6 workdays: Mon..Sat
            const days = (0, date_1.weekDaysMonToSat)(anchorISO);
            if (!days)
                return res.status(400).json({ error: "INVALID_DATE" });
        ...
        `;
        const params = [globalStart, globalEnd];
        if (targetWorkerId)
            params.push(targetWorkerId);
        const raw = await (0, db_1.query)(sql, params);
        ...
        const tasks = raw.rows.map(t => {
            ...
            if (vDate instanceof Date) {
                vDate = luxon_1.DateTime.fromJSDate(vDate).setZone(date_1.APP_TZ).toISODate();
            }
            else if (typeof vDate === 'string') {
                if (vDate.includes('T')) {
                    vDate = luxon_1.DateTime.fromISO(vDate).setZone(date_1.APP_TZ).toISODate();
                }
                // If it's already "YYYY-MM-DD", it stays as is
            }
            let aDate = t.assigned_date;
            if (aDate instanceof Date) {
                aDate = luxon_1.DateTime.fromJSDate(aDate).setZone(date_1.APP_TZ).toISODate();
            }
            else if (typeof aDate === 'string' && aDate.includes('T')) {
                aDate = luxon_1.DateTime.fromISO(aDate).setZone(date_1.APP_TZ).toISODate();
            }
 ...
 */
exports.adminRouter.get("/reports/full", (0, auth_1.requireRole)("admin"), async (req, res, next) => {
    try {
        const qv = (0, validate_1.mustParse)(zod_1.z.object({
        ...
        const r = await (0, db_1.query)(`SELECT t.*, u.full_name as worker_name,
             ...
             ORDER BY u.full_name ASC, t.visible_date ASC, t.is_mandatory DESC, t.created_at ASC`, [qv.start, qv.end]);
        ...
        for (const row of r.rows) {
            ...
            if (row.visible_date instanceof Date) {
                row.visible_date = luxon_1.DateTime.fromJSDate(row.visible_date).setZone(date_1.APP_TZ).toISODate();
            }
