import React, { useEffect, useMemo, useRef } from "react";
import DayCard9x16 from "./DayCard9x16";

export default function WeekSwiper6Cards({
    days,
    data,
    anchor,
    mode,
    onChangeAnchor,
    onRefresh
}: {
    days: string[];
    data: Record<string, any>;
    anchor: string;
    mode: "week" | "day";
    onChangeAnchor: (iso: string) => void;
    onRefresh: () => void;
}) {
    const containerRef = useRef<HTMLDivElement | null>(null);

    const activeIndex = useMemo(() => {
        const idx = days.indexOf(anchor);
        return idx >= 0 ? idx : 0;
    }, [anchor, days]);

    useEffect(() => {
        if (!containerRef.current) return;
        const el = containerRef.current;
        const child = el.children.item(activeIndex) as HTMLElement | null;
        if (child) child.scrollIntoView({ behavior: "smooth", inline: "center", block: "nearest" });
    }, [activeIndex]);

    const visibleDays = mode === "day" ? [anchor] : days;

    return (
        <div className="weekWrap">
            <div className={mode === "week" ? "weekGrid" : "weekStrip"} ref={containerRef}>
                {visibleDays.map((d) => (
                    <div key={d} className="daySlot">
                        <DayCard9x16
                            dayISO={d}
                            group={data[d] || { mandatory: [], normal: [], carryover: [], progress: { done: 0, total: 0, percent: 0 } }}
                            onPickDay={onChangeAnchor}
                            onRefresh={onRefresh}
                        />
                    </div>
                ))}
            </div>

            {mode !== "week" && (
                <div className="weekDots">
                    {days.map((d) => (
                        <button
                            key={d}
                            className={`dot ${d === anchor ? "dotActive" : ""}`}
                            onClick={() => onChangeAnchor(d)}
                            aria-label={d}
                        />
                    ))}
                </div>
            )}
        </div>
    );
}
